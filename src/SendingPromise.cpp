...
SendingPromise& SendingPromise::setRange(const SmartIotRange& range) {
  _range = range;
  return *this;
}
...
uint16_t SendingPromise::send(const String& value) {
  ...
  if (_range.isRange) {
    ...
    _range.isRange = false;                  //FIXME: This is a workaround. Problem is that Range is loaded from the property into SendingPromise, but the SendingPromise is global. (one SendingPromise for the SmartIotClass instance
    _range.index = 0;
  }
...
SendingPromise& SendingPromise::setNode(const SmartIotNode& node) {
  _node = &node;
  return *this;
}
...
const SmartIotNode* SendingPromise::getNode() const {
  return _node;
}
...
SmartIotRange SendingPromise::getRange() const {
  return _range;
}
