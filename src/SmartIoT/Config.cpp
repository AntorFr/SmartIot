...
bool Config::load() {
  ...
  if (!SPIFFS.exists(SETUP_FILE_PATH)) {
    Interface::get().getLogger() << F("✖ ") << SETUP_FILE_PATH << F(" doesn't exist") << endl;
    return false;
  }

  File configFile = SPIFFS.open(SETUP_FILE_PATH, "r");
  if (!configFile) {
    Interface::get().getLogger() << F("✖ Cannot open setup file") << endl;
    return false;
  }
  ...
  if (configSize >= MAX_JSON_CONFIG_FILE_SIZE) {
    Interface::get().getLogger() << F("✖ setup file too big") << endl;
    return false;
  }
  ...
  const char* reqName = parsedJson["name"] | DeviceId::getChipId();
  ...
  bool reqOtaEnabled = parsedJson["ota"]["enabled"] | true;
...
void Config::setSmartIotBootModeOnNextBoot(SmartIotBootMode bootMode) {
  if (!_spiffsBegin()) { return; }

  if (bootMode == SmartIotBootMode::UNDEFINED) {
    SPIFFS.remove(CONFIG_NEXT_BOOT_MODE_FILE_PATH);
  } else {
...
SmartIotBootMode Config::getSmartIotBootModeOnNextBoot() {
  if (!_spiffsBegin()) { return SmartIotBootMode::UNDEFINED; }

  File bootModeFile = SPIFFS.open(CONFIG_NEXT_BOOT_MODE_FILE_PATH, "r");
  if (bootModeFile) {
    int v = bootModeFile.parseInt();
    bootModeFile.close();
    return static_cast<SmartIotBootMode>(v);
  } else {
    return SmartIotBootMode::UNDEFINED;
  }
}
...
void Config::log() const {
  ...
  if (ISmartIotSetting::settings.size() > 0) {
