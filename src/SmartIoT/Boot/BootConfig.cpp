...
void BootConfig::_onCaptivePortal(AsyncWebServerRequest *request) {
  ...
  if (host && !host.equals(_apIpStr)) {
    ...
    // UI File not found
    String msg = String(F("UI bundle not loaded. See Configuration API usage: http://SmartIoTiot.github.io/SmartIoT-esp8266"));
...
void BootConfig::_proxyHttpRequest(AsyncWebServerRequest *request) {
  ...
  _httpClient.setUserAgent(F("ESP8266-SmartIot"));
...
void SmartIotInternals::BootConfig::__SendJSONError(AsyncWebServerRequest * request, String msg, int16_t code) {
  Interface::get().getLogger() << msg << endl;
