...
ota firmware update scirpt for ESP8226 implemenation of the SmartIot mqtt IoT
convention.
...
arguments:
  ...
  -t BASE_TOPIC, --base-topic BASE_TOPIC
                        base topic of the SmartIoT devices on the broker
...
* `BASE_TOPIC` has to end with a slash, defaults to `SmartIoT/` if not set.

### Example:

```bash
python ota_updater.py -l localhost -u admin -d secure -t "SmartIoT/" -i "device-id" /path/to/firmware.bin
```
